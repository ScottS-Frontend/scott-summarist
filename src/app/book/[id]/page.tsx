...
import { loadSubscription, clearSubscription } from "@/store/subscriptionSlice";
...
export default function BookDetailPage() {
  ...
  // Clear subscription when user changes, then load new one
  useEffect(() => {
    console.log("BookDetail - user changed:", user?.email, user?.uid);
...
console.log("isPremiumUser:", subscription?.status === "active");
console.log("book?.subscriptionRequired:", book?.subscriptionRequired);

  const isGuest = user?.email === "guest@gmail.com";
  const isPremiumUser = !isGuest && subscription?.status === "active";
          ...
          <div className="lg:hidden">
            <div className="w-full flex justify-center mb-6">
              <div className="relative">
                ...
                <BookImage
                  book={book}
                  className="w-[280px] h-[380px] lg:w-[320px] lg:h-[440px]"
            ...
            <div>
              ...
              >
                <BsBookmark className={`w-5 h-5 ${isBookSaved ? "fill-[#0365f2] text-[#0365f2]" : ""}`} />
                <span className="text-sm">
                  {isBookSaved ? "In My Library" : isAdding ? "Processing..." : "Add title to My Library"}
                </span>
              </button>
              ...
              <p className="text-gray-700 leading-relaxed mb-10 text-sm">{book.bookDescription}</p>

              <h2 className="text-lg font-bold text-[#032b41] mb-4">About the author</h2>
              <p className="text-gray-700 leading-relaxed text-sm">{book.authorDescription}</p>
            </div>
          </div>
          ...
          <div className="hidden lg:flex gap-12">
            <div className="flex-1">
              <h1 className="text-3xl font-bold text-[#032b41] mb-2">
                {book.title}
                {showPremiumLabel && <span className="text-gray-400 font-normal ml-2">(Premium)</span>}
              </h1>
              ...
              <button
                ...
                className={`flex items-center gap-2 text-[#0365f2] hover:text-blue-700 mb-10 transition-colors font-bold cursor-pointer ${isAdding ? "opacity-50 cursor-not-allowed" : ""}`}
              >
                {isBookSaved ? <BsBookmarkFill className="w-5 h-5 text-[#0365f2]" /> : <BsBookmark className="w-5 h-5 text-[#0365f2]" />}
                <span className="text-sm">
                  {isBookSaved ? "In My Library" : isAdding ? "Processing..." : "Add title to My Library"}
                </span>
              </button>
            ...
            <div className="w-80 flex-shrink-0 relative">
              ...
              <BookImage book={book} className="w-full h-[420px]" showHoverEffect={false} />
            </div>
          </div>
        </div>
      </main>
